...
import { RegisterUserUseCase } from "../../../core";
...
export const RegisterApp = () => {
  ...
  useEffect(() => {
    setValidSend(!emailError && !nameError && !passwordError && email !== '' && password !== '' && name !== '');
  ...
  const onSubmit = async(e: FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    const data = await RegisterUserUseCase(name, email, password);
    setPassword('');
    ...
    <div className='mt-32'>
      ...
      >
        ...
        <div className='flex flex-col gap-1 text-black mb-4'>
          ...
          <input value={ name } onChange={ onChangeName } type='text' className={`focus:outline-indigo-600 shadow-sm bg-white px-3 py-2 rounded-lg`} id='name' placeholder='hello123'></input>
          {
            nameError
            &&
            <AlertFormApp content={nameError} error/>
          }
        </div>
